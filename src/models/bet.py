...
class BeT(nn.Module):
    def __init__(
        ...
        self._train = True if actions is not None else False  # controls output behavior during forward pass
        self.device = device

        # Initializing the Encoder Decoder which is based on the K-Means
        self.encoderDecoder = EncoderDecoder(
            action_dim, num_bins, actions, device
        )
    ...
    def forward(self, observations_history):
        ...
        predicted_action = self.encoderDecoder.decode(
            selected_seq_action_bins[:, -1, 0], selected_seq_action_residuals[:, -1, :]
        )  # selects only the last element of the sequence
...
if __name__ == "__main__":
    ...
    observations_history = torch.rand(
